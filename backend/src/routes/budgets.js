...
router.get(
  ...
  wrapAsync(async (req, res) => {
    ...
    const payload = await BudgetsDBApi.findAll(req.query, { currentUser });
    if (filetype && filetype === 'csv') {
      const fields = ['id', 'year', 'total_budget', 'expenses'];
      const opts = { fields };
      try {
...
router.get(
  ...
  wrapAsync(async (req, res) => {
    const currentUser = req.currentUser;
    const payload = await BudgetsDBApi.findAll(req.query, null, {
      countOnly: true,
      currentUser,
    });
