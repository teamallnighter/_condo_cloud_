...
const CardNotices = ({
  ...
  const hasUpdatePermission = hasPermission(currentUser, 'UPDATE_NOTICES');
        ...
        {!loading &&
          notices.map((item, index) => (
            ...
            >
              ...
              >
                <Link
                  href={`/notices/notices-view/?id=${item.id}`}
                ...
                <div className='ml-auto '>
                  <ListActionsPopover
                    onDelete={onDelete}
                    itemId={item.id}
                    pathEdit={`/notices/notices-edit/?id=${item.id}`}
                    pathView={`/notices/notices-view/?id=${item.id}`}
              ...
              <dl className='divide-y  divide-stone-300   dark:divide-dark-700 px-6 py-4 text-sm leading-6 h-64 overflow-y-auto'>
                <div className='flex justify-between gap-x-4 py-3'>
                  ...
                  <dd className='flex items-start gap-x-2'>
                    <div className='font-medium line-clamp-4'>
                      {dataFormatter
                        .usersManyListFormatter(item.recipients)
                        .join(', ')}
                    </div>
                  </dd>
                </div>
                ...
                <div className='flex justify-between gap-x-4 py-3'>
                  ...
                  <dd className='flex items-start gap-x-2'>
                    <div className='font-medium line-clamp-4'>
                      {dataFormatter.dateTimeFormatter(item.sent_date)}
                    </div>
                  </dd>
                </div>
              </dl>
