...
export default {
  ...
  unitsManyListFormatter(val) {
    if (!val || !val.length) return [];
    return val.map((item) => item.unit_number);
  },
  unitsOneListFormatter(val) {
    if (!val) return '';
    return val.unit_number;
  },
  unitsManyListFormatterEdit(val) {
    if (!val || !val.length) return [];
    return val.map((item) => {
      return { id: item.id, label: item.unit_number };
    });
  },
  unitsOneListFormatterEdit(val) {
    if (!val) return '';
    return { label: val.unit_number, id: val.id };
  },
